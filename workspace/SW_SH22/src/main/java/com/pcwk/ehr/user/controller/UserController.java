...
public class UserController {   
   ...
   public String add(UserVO inVO) throws SQLException{
      ...
      if(1 == flag) {
         resultMessaage = inVO.getuId() + "가 등록되었습니다";
      }else {
         resultMessaage = inVO.getuId() + "가 등록 실패";
      }
      MessageVO message = new MessageVO(String.valueOf(flag), resultMessaage);
      Gson gson = new Gson();
   ...
   public String doDelete(HttpServletRequest req, Model model, UserVO inVO) throws SQLException{
      ...
      LOG.debug("=uId="+uId);
      //Command로 param읽기(from name 값이 VO member변수와 이름이 일치(setter로 값이 설정)
      LOG.debug("=inVO="+inVO);
      LOG.debug("==============================");
